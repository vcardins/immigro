    ...
    configure() {       
        ...
        RequestBuilder.addHelper('authTokenHandling', (internalAuth:boolean = true, identifier:any = undefined)=>{
			return (client, processor, message)=>{
				if (this.authProvider.isAuthenticated && this.appSettings.httpInterceptor) {					                    					
                    if (internalAuth) {
                        message.interceptors = message.interceptors || [];
                        message.interceptors.unshift(new AuthenticationInterceptor(this.authProvider, this.logger));
                    }                                                            
